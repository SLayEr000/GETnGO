...
- (void)scrollToNearlyIndexAtDirection:(TYPagerScrollDirection)direction animate:(BOOL)animate {
    if (direction == TYPagerScrollDirectionRight && self.curIndex == self.numberOfItems - 1) {
        return;
    }
...
- (void)scrollToItemAtIndexSection:(TYIndexSection)indexSection animate:(BOOL)animate {
    ...
    if (self.isLogEnabled) {
        NSLog(@"offset ===%f animate = %d", offset, animate);
    }
...
//    DLog(@"scrollView.contentOffset.x = %f targetContentOffset->x=%f scrollView.contentSize.width = %f scrollView.frame.size.width = %f", scrollView.contentOffset.x, targetContentOffset->x, scrollView.contentSize.width, scrollView.frame.size.width);
    ...
    if (self.isLogEnabled) {
        NSLog(@"_indexSection.index ===%ld _numberOfItems = %ld direction = %lu indexSection.index = %ld", (long)_indexSection.index,(long)_numberOfItems, (unsigned long)direction, (long)indexSection.index);
    }
    if (_indexSection.index == _numberOfItems - 1 && indexSection.index == 0 && scrollView.contentOffset.x == scrollView.contentSize.width - scrollView.frame.size.width) { //禁止最右侧继续往右滚动
        return;
    }
    if (_indexSection.index == 0 && indexSection.index == 1 && scrollView.contentOffset.x == 0) { //禁止最左侧继续往左滚动
        return;
    }
    targetContentOffset->x = [self caculateOffsetXAtIndexSection:indexSection];
    if (self.isLogEnabled) {
        NSLog(@"run here targetContentOffset ===%f direction ===%lu indexSection.index = %ld indexSection.section = %ld", targetContentOffset->x, (unsigned long)direction, (long)indexSection.index, (long)indexSection.section);
    }
}
